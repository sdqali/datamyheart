...
var boxPlotWidth = 65 - margin.left - margin.right;
var boxPlotHeight = 500 - margin.top - margin.bottom;
...
d3.csv("../real_data.csv", function(d) {
    ...
    var min = d3.min(juices.map(function(j){
        return j.min;
    }));

    var max = d3.max(juices.map(function(j){
        return j.max;
    }));

    var juiceCounts = juices.map(function(j){
        return j.values;
    });

    var boxData = juiceCounts.map(function(jc) {
        return d3.values(jc).map(function(j){
            return j.values;
        });
    });
    ...
    var vis = d3.select("#boxplot").selectAll("svg")
        ...
        .attr("height", boxPlotHeight + margin.bottom + margin.top)
        .append("g")
